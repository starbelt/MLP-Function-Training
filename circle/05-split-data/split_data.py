...
shutil.copy(os.path.join(src,'circle-npy-to-cfg.json'), dst_trn)
shutil.copy(os.path.join(src,'circle-npy-to-cfg.json'), dst_val)
shutil.copy(os.path.join(src,'circle-npy-to-cfg.json'), dst_tst)
...
if len(sys.argv) != 3:
    print("Usage: python3 split_data.py /path/to/src /path/to/dst")
    exit()
...
n_total = len(npys)
n_trn = int(TRN_FRAC * n_total)
n_val = int(VAL_FRAC * n_total)

trn_files = npys[:n_trn]
val_files = npys[n_trn:n_trn + n_val]
tst_files = npys[n_trn + n_val:]

def copy_files(files, dst_dir):
    for f in files:
        shutil.copy(os.path.join(src, f), dst_dir)

copy_files(trn_files, dst_trn)
copy_files(val_files, dst_val)
copy_files(tst_files, dst_tst)
