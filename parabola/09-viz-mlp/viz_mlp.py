...
for npy in npys:
  parabola_id = npy[:-4]
  data_cfg = np.array([
   npy_to_cfg_dict[parabola_id]['a'],
   npy_to_cfg_dict[parabola_id]['b'],
   npy_to_cfg_dict[parabola_id]['c']
  ])
  ...
  print(f"{parabola_id}: MSE={mse:.6f}, MAE={mae:.6f}, MaxErr={max_err:.6f}")

  plt_title = \
   'a term: '+'{:.3f}'.format(npy_to_cfg_dict[parabola_id]['a'])+'; '+\
   'b term: '+'{:.3f}'.format(npy_to_cfg_dict[parabola_id]['b'])+'; '+\
   'c term: '+'{:.3f}'.format(npy_to_cfg_dict[parabola_id]['c'])
  plt_y_axis = 'f(x)'
  plt_x_axis = 'x'
  ...
  plt.legend()
  plt.savefig(os.path.join(dst,parabola_id+'.png'),format='png')
  plt.close(fig)
